...
import xyz.skywind.raft.utils.RaftImplementationException
import xyz.skywind.tools.Logging

class InMemoryOperationLog(nodeID: NodeID) : OperationLog {
    ...
    override fun append(leaderLastEntry: LogEntryInfo, newOperations: List<Operation>) {
        if (leaderLastEntry.index == operations.lastIndex) {
            ...
            throw RaftImplementationException("Can't find prev entry $leaderLastEntry in log{size=${operations.size}}")
        }
    }

    override fun getEntryAt(index: Int): LogEntryInfo {
        if (index < 0 || index >= operations.size) {
            throw RaftImplementationException("Accessing out-of-bounds index $index. Log: ${toDetailedString()}")
        }
